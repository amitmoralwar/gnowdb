...
(defresource create-Class [request]
  :service-available? true
  :allowed-methods [:post]
  :handle-created (fn [_] (let [params (get-in request[:params])] (createClass :className (get-in params [:className])
                                                                               :classType (get-in params [:classType])
                                                                               :isAbstract? (get-in params [:isAbstract?])
                                                                               :subClassOf (get-in params [:subClassOf])
                                                                               :properties (get-in params [:properties])
                                                                               :execute? (get-in params [:execute?]))))
  :available-media-types ["application/json"])

(defresource create-Node-Class-Instances [request]
  :service-available? true
  :allowed-methods [:post]
  :handle-created (fn [_] (let [params (get-in request[:params])] (createNodeClassInstances :className (get-in params [:className])
                                                                                            :nodeList (get-in params [:nodeList])
                                                                                            :execute? (if (contains? params :execute?) (get-in params [:execute?]) true))))
  :available-media-types ["application/json"])

(defresource create-Relation-Class-Instances [request]
  :service-available? true
  :allowed-methods [:post]
  :handle-created (fn [_] (let [params (get-in request[:params])] (createRelationClassInstances :className (get-in params [:className])
                                                                                            :relList (get-in params [:relList])
                                                                                            :execute? (if (contains? params :execute?) (get-in params [:execute?]) true))))
  :available-media-types ["application/json"])
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
